...
# related to geographic properties (i.e. it has nothing to do with HSPF per se).
...
# I have assumed in my analysis that landuse can change every year, but it's 
# not necessary for HSPF. If you want to have impervious land, you must use
# "Developed" to identify it (this was not very pythonic of me); later you
# can change the relative percentages.
...
# the UCI file name will be 'example.uci'
...
# output file (we'll need this later)
...
# first let's calculate number of 4-hr time steps in the simulation

n = int((end - start).total_seconds() / 3600 / 4)
...
# evaporation is 2 mm. obviously the data should come from a reliable source.

jan_june = [4 * i / n for i in range(n//2)]

# make a copy and reverse it (note you CANNOT do july_dec = jan_jane)
...
# landuse category, or even each operation if you want.
