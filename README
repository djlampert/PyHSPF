...
National Climate Data Center, and landuse data from the National 
...
Pillow in addition to NumPy, Scipy, and Matplotlib, and make a series 
...
files. A series of examples are provided to illustrate PyHSPF core module 
usage.

The preprocessing modules contain a powerful calculator to automate 
extraction of climate data from various databases including the National 
Solar Radiation Database (NSRDB), Global Historical Climate Network Daily 
(GHCND), Global Summary of the Day (GSOD), and the NCDC Hourly Precipitation 
Database (DSI-3240). The preprocessing modules can calculate daily or 
hourly potential evapotranspiration time series using the ASCE 
Penman-Monteith Equation. This is a major difference from the other 
databases (namely EPA Basins which uses the Hammond method). The Penman
calculator could be used completely outside PyHSPF.
